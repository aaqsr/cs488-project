...
template <int numRows, int numCols>
inline WaterMesh<numRows, numCols>::WaterMesh(
...
template <int numRows, int numCols>
inline void WaterMesh<numRows, numCols>::updateMesh(
  const std::array<float, numRows * numCols>& heights)
{
    // upload only the height data
    glBindBuffer(GL_ARRAY_BUFFER, heightVBO);

    // orphan old buffer so that we don't have to wait for the GPU to be done
    // with it GPU *should* clean up the old buffer for us/
...
template <int numRows, int numCols>
inline void WaterMesh<numRows, numCols>::draw(Shader::BindObject& shader) const
{
