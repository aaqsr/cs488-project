 ...
 *
 * @section Technicality
 * This class implements a handwritten **recursive descent parser**.
 *
 * The `Lexer` performs lexical analysis, converting the raw source string into
 * a sequence of tokens (e.g., `LeftBrace`, `String`, `Number`). The `Parser`
 * then consumes these tokens, building the hierarchical `JsonValue` structure
 * by recursively calling private parsing functions for each language construct
