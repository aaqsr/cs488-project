...
PhysicsObj::PhysicsObj(Model& m): model{m}, prevPos{m.worldPos} {
...
void PhysicsObj::calcInertia() {
    auto inertia = Inertia();
    for (const Mesh& mesh: model.getMeshes()) {
        for (size_t face_idx = 2; face_idx < mesh.getFaces().size(); face_idx += 3) {
            ...
            // process faces
            inertia.add_face(v0.position, v1.position, v2.position, 1.0f);
        }
    }
